...
class AggregationService:
    ...
    def aggregate_combined(
        ...
        if familia:
            ...
            if df_filtered.empty:
                raise ValueError(f"Família(s) '{', '.join(familia)}' não encontrada(s) nos dados")
            filters_applied.append(f"Familia={', '.join(familia)}")
            print(f"🔍 Filtro aplicado: Familia = {', '.join(familia)} → {len(df_filtered)} registros")
        
        if processo:
            df_filtered = df_filtered[df_filtered["Processo"].isin(processo)]
            if df_filtered.empty:
                raise ValueError(f"Processo(s) '{', '.join(processo)}' não encontrado(s) nos dados (após filtros anteriores)")
            filters_applied.append(f"Processo={', '.join(processo)}")
            print(f"🔍 Filtro aplicado: Processo = {', '.join(processo)} → {len(df_filtered)} registros")
        ...
        if abc_class:
            ...
            if invalid_classes:
                raise ValueError(f"Classe(s) ABC inválida(s): '{', '.join(invalid_classes)}'. Use A, B ou C")
            
            skus_abc = df_classified[df_classified["Classe_ABC"].isin(abc_class_upper)]["SKU"].unique()
            
            if len(skus_abc) == 0:
                raise ValueError(f"Nenhum SKU encontrado para classe(s) ABC '{', '.join(abc_class_upper)}'")
            
            df_filtered = df_filtered[df_filtered["SKU"].isin(skus_abc)]
            
            if df_filtered.empty:
                raise ValueError(f"Nenhum dado encontrado para classe(s) ABC '{', '.join(abc_class_upper)}' (após filtros anteriores)")
            
            filters_applied.append(f"ABC={', '.join(abc_class_upper)}")
            print(f"🔍 Filtro aplicado: ABC = {', '.join(abc_class_upper)} → {len(df_filtered)} registros")
    ...
    @staticmethod
    def aggregate_familia(df: pd.DataFrame, familia: Optional[List[str]] = None) -> Tuple[pd.DataFrame, dict]:
        df_clean = AggregationService._prepare_data(df)
    ...
    @staticmethod
    def aggregate_by_processo(df: pd.DataFrame, processo: Optional[List[str]] = None) -> Tuple[pd.DataFrame, dict]:
        df_clean = AggregationService._prepare_data(df)
        
        if processo:
            df_filtered = df_clean[df_clean["Processo"].isin(processo)].copy()
            if df_filtered.empty:
                raise ValueError(f"Processo(s) '{', '.join(processo)}' não encontrado(s) nos dados")
    ...
    def aggregate_by_abc(
        ...
        if df_filtered.empty:
            raise ValueError(f"Nenhum dado encontrado para classe(s) ABC '{', '.join(abc_class_upper)}'")
        ...
        print(f"📊 Agregação ABC concluída: {len(df_aggregated)} períodos, {len(skus_abc)} SKUs da(s) classe(s) {', '.join(abc_class_upper)}")
    ...
    def aggregate_all(df: pd.DataFrame) -> Tuple[pd.DataFrame, dict]:
        ...
        print(f"📊 Agregação total concluída: {len(df_aggregated)} períodos, {info['skus_count']} SKUs")
