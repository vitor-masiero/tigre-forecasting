...
class ForecastRequest(BaseModel):
    ...
    model: Optional[str] = Field(
        default=None, description="Modelo específico (Prophet, TSB, XGBoost). Se None, usa redirecionamento ABC para SKU individual"
    )
    
    # Campos de agregação
    aggregation_type: Literal["sku", "familia", "processo", "abc", "all", "combined"] = Field(
        default="sku", 
    ...
    familia: Optional[str] = Field(
        default=None, description="Filtro por família/linha específica"
    )

    processo: Optional[str] = Field(
        default=None, description="Filtro por processo específico"
    )

    abc_class: Optional[str] = Field(
        default=None, description="Filtro por classe ABC (A, B ou C)"
    )
    ...
    def validate_model(cls, v):
        if v is not None:
            ...
            if v not in valid_models:
                raise ValueError(f"Modelo inválido. Use um dos seguintes: {', '.join(valid_models)}")
        return v
    ...
    def validate_abc_class(cls, v):
        if v is not None:
            v = v.upper().strip()
            if v not in ["A", "B", "C"]:
                raise ValueError("Classe ABC deve ser A, B ou C")
        return v
...
class ForecastRunResponse(BaseModel):
    ...
    aggregation_info: Optional[dict] = Field(
        default=None, 
        description="Informações sobre a agregação realizada"
    )
