...
class ForecastRequest(BaseModel):
    ...
    familia: Optional[List[str]] = Field(
        default=None, description="Filtro por família/linha específica (pode conter múltiplos valores)"
    )

    processo: Optional[List[str]] = Field(
        default=None, description="Filtro por processo específico (pode conter múltiplos valores)"
    )

    abc_class: Optional[List[str]] = Field(
        default=None, description="Filtro por classe ABC (A, B ou C) (pode conter múltiplos valores)"
    )
        ...
        if v is not None:
            ...
            for item in v:
                ...
                if item_upper not in ["A", "B", "C"]:
                    raise ValueError(f"Classe ABC '{item}' inválida. Deve ser A, B ou C")
