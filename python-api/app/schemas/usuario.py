...
class UsuarioBase(BaseModel):
    ...
    role: UserRole = Field(
        ..., description="Perfil do usuário (analista, comercial, gestao)"
    )
...
class UsuarioCreate(UsuarioBase):
    ...
    def validate_role(cls, v):
        if not UserRole.has_value(v):
            raise ValueError(
                f"Role inválido. Use um dos seguintes: {', '.join(UserRole.list_values())}"
            )
        return v
...
class UsuarioListResponse(BaseModel):
    ...
    base_admin: Optional[dict] = Field(
        None, description="Informações do usuário base protegido"
    )
...
class UsuarioDeleteResponse(BaseModel):
    ...
    protected_warning: Optional[str] = Field(
        None, description="Aviso se houver proteção"
    )
