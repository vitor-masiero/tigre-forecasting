...
def criar_usuario_base():
    ...
    with DatabaseConfig.get_db_session() as db:
        ...
        print(f"   Nome: {nome}")
        print(f"   Email: {email}")
        print(f"   Role: gestao (Gestor)")
        print(f"   Status: Ativo")
        print(f"   Proteção: SIM (não pode ser excluído/desativado)")
        print("-"*70 + "\n")
        
        confirmacao = input("❓ Confirma a criação? Digite 'SIM' para continuar: ").strip().upper()
        ...
        try:
            ...
            print(f"   🔒 Protegido: SIM (não pode ser excluído/desativado)")
            print("="*70)
